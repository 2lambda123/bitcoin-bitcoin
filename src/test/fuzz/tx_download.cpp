...
namespace {
// What a peer may send unsolicited.
...
// A non-honest peer can do everything the honest peer might do, but also some other things.
...
public:
    ...
    PossibleMessage SendOneMessage(FuzzedDataProvider& fuzzed_data_provider, bool force_announce = false) {
        // Honest peers send the inv at most once, and otherwise will not send an unsolicited message.
        if (m_honest) {
        ...
        // Non-honest peers might send anything!
    ...
    PossibleResponse ProcessOneMessage(FuzzedDataProvider& fuzzed_data_provider) {
        ...
        if (!m_honest) response = fuzzed_data_provider.PickValueInArray(ALL_RESPONSES);
...
// honest_only means necessarily honest.
TestPeer RandomPeer(FuzzedDataProvider& fuzzed_data_provider, NodeId nodeid, bool preferred, bool honest_only)
{
    TestPeer peer;
    peer.m_nodeid = nodeid;
    peer.m_honest = honest_only;
    peer.m_connection_preferred = preferred;
    peer.m_relay_permissions = false;
    peer.m_wtxid_relay = honest_only || fuzzed_data_provider.ConsumeBool();
    return peer;
}
...
// Connect to 8 outbounds and 0-120 inbound peers. 1 outbound is guaranteed to be honest, while the
// rest may or may not be; we assert that we always end up downloading the transaction.
// An honest node sends an announcement for the tx at some point and will respond to getdata with
// the correct transaction.
// A non-honest node has a superset of the actions available to the honest node: it may stall a
// getdata, send a malleated or witness stripped tx/inv, send a notfound, etc. Note that, in the
// real world, notfounds are normal and expected (e.g. if the tx is evicted or replaced in between
// inv and getdata). However, for this test, we are only interested in honest peers that don't
// forget the transaction (otherwise we really cannot expect to download it).
FUZZ_TARGET(tx_download, .init = initialize)
{
    ...
    // NodeId of the "designated honest peeer" that will never be disconnected. We'll start counting
    // after this peer has announced the tx.
    NodeId honestid = fuzzed_data_provider.ConsumeIntegralInRange<unsigned int>(0, 7);
    // 8 outbound peers. NodeId = o.
    for (unsigned int o = 0; o < 8; ++o) {
        // all_peers.emplace_back(RandomPeer(fuzzed_data_provider, o, /*preferred=*/true, /*honest_only=*/(o == honestid)));
        all_peers.emplace_back(RandomPeer(fuzzed_data_provider, o, /*preferred=*/true, /*honest_only=*/true));
    }
    // 0-120 inbound peers. NodeId = i.
    ...
    for (unsigned int i = 8; i < num_inbound_peers + 8; ++i) {
        all_peers.emplace_back(RandomPeer(fuzzed_data_provider, i, /*preferred=*/false, /*honest_only=*/false));
    }
    ...
    // We are trying to ensure that we always download the tx as long as we have 1 honest outbound
    ...
    LIMITED_WHILE(!got_tx, 100000)
    {
        ...
        for (const auto index : indexes) {
            ...
            // Maybe connect the peer if we haven't already. Always connect if it's the honest peer.
            if (nodeid == honestid ||
            ...
            if (peer.m_connection_state == ConnectionState::CONNECTED) {
                ...
                // Ensure the honest peer sends the inv within the first 1000 rounds.
                ...
                // Maybe disconnect the peer if we haven't already. Never disconnect The honest peer.
