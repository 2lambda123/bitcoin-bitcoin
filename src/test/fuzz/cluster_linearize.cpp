        ...
        if (depgraph.TxCount() <= 7) {
            ...
            do {
                ...
                for (auto i : perm_linearization) {
                    perm_done.Set(i);
                    if (!(depgraph.Ancestors(i) << perm_done)) {
                        perm_is_topo = false;
                        break;
                    }
                }
...
{
    // Verify expected properties of PostProcess() on arbitrary linearizations.
    ...
    // The chunks that come out of postprocessing are always connected.
    ClusterIndex idx = 0;
...
{
    // Verify expected properties of PostProcess() on linearizations of graphs that form either an
    // upright or reverse tree structure.
