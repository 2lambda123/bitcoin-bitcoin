                                                             ...
                                                             std::chrono::seconds average_interval)
{
    if (m_next_inv_to_inbounds.load() < now) {
        ...
        m_next_inv_to_inbounds = now + m_rng.rand_expo_duration(average_interval);
    }
...
void PeerManagerImpl::MaybeSendAddr(CNode& node, Peer& peer, std::chrono::microseconds current_time)
{
    ...
    if (fListen && !m_chainman.IsInitialBlockDownload() &&
        ...
        peer.m_next_local_addr_send = current_time + m_rng.rand_expo_duration(AVG_LOCAL_ADDRESS_BROADCAST_INTERVAL);
    }

    // We sent an `addr` message to this peer recently. Nothing more to do.
    if (current_time <= peer.m_next_addr_send) return;

    peer.m_next_addr_send = current_time + m_rng.rand_expo_duration(AVG_ADDRESS_BROADCAST_INTERVAL);
...
void PeerManagerImpl::MaybeSendFeefilter(CNode& pto, Peer& peer, std::chrono::microseconds current_time)
{
    ...
    if (current_time > peer.m_next_send_feefilter) {
        ...
        peer.m_next_send_feefilter = current_time + m_rng.rand_expo_duration(AVG_FEEFILTER_BROADCAST_INTERVAL);
    }
        ...
        if (auto tx_relay = peer->GetTxRelay(); tx_relay != nullptr) {
                ...
                if (tx_relay->m_next_inv_send_time < current_time) {
                    ...
                    if (pto->IsInboundConn()) {
                        ...
                        tx_relay->m_next_inv_send_time = current_time + m_rng.rand_expo_duration(OUTBOUND_INVENTORY_BROADCAST_INTERVAL);
                    }
                }
