...
void PeerManagerImpl::Misbehaving(Peer& peer, int howmuch, const std::string& message)
{
    ...
    LogDebug(BCLog::NET, "Misbehaving: peer=%d (%d -> %d)%s%s\n",
        ...
        const std::vector<CBlockHeader>& headers)
{
    ...
    if (MaybeSendGetHeaders(pfrom, GetLocator(best_header), peer)) {
        LogDebug(BCLog::NET, "received header %s: missing prev block %s, sending getheaders (%d) to end (peer=%d, m_num_unconnecting_headers_msgs=%d)\n",
        ...
        if (result.request_more) {
            ...
            if (!locator.vHave.empty()) {
                ...
                if (sent_getheaders) {
                    ...
                    LogDebug(BCLog::NET, "error sending next getheaders (from %s) to continue sync with peer=%d\n",
                            locator.vHave.front().ToString(), pfrom.GetId());
                }
            }
        }
        ...
        const CBlockIndex& last_header, bool received_new_header, bool may_have_more_headers)
{
    if (peer.m_num_unconnecting_headers_msgs > 0) {
        LogDebug(BCLog::NET, "peer=%d: resetting m_num_unconnecting_headers_msgs (%d -> 0)\n", pfrom.GetId(), peer.m_num_unconnecting_headers_msgs);
    }
