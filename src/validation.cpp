...
bool MemPoolAccept::PreChecks(ATMPArgs& args, Workspace& ws)
{
    ...
    for (const CTxIn &txin : tx.vin)
    {
        ...
        if (ptxConflicting) {
            ...
            if (!ws.m_conflicts.count(ptxConflicting->GetHash()))
            {
                ...
                const bool allow_rbf{m_pool.m_opts.full_rbf || SignalsOptInRBF(*ptxConflicting) || ptxConflicting->version == 3};
                if (!allow_rbf) {
    ...
    if (!bypass_limits && ws.m_ptx->version != 3 && ws.m_modified_fees < m_pool.m_opts.min_relay_feerate.GetFee(ws.m_vsize)) {
