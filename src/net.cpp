...
void CConnman::ThreadOpenConnections(const std::vector<std::string> connect)
{
    ...
    auto next_feeler = start + rng.rand_expo_duration(FEELER_INTERVAL);
    auto next_extra_block_relay = start + rng.rand_expo_duration(EXTRA_BLOCK_RELAY_ONLY_PEER_INTERVAL);
    auto next_extra_network_peer{start + rng.rand_expo_duration(EXTRA_NETWORK_PEER_INTERVAL)};
    ...
    while (!interruptNet)
    {
        ...
        if (!m_anchors.empty() && (nOutboundBlockRelay < m_max_outbound_block_relay)) {
            ...
            next_extra_block_relay = now + rng.rand_expo_duration(EXTRA_BLOCK_RELAY_ONLY_PEER_INTERVAL);
            conn_type = ConnectionType::BLOCK_RELAY;
        } else if (now > next_feeler) {
            next_feeler = now + rng.rand_expo_duration(FEELER_INTERVAL);
            ...
            next_extra_network_peer = now + rng.rand_expo_duration(EXTRA_NETWORK_PEER_INTERVAL);
        } else {
