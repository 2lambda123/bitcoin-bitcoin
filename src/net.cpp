...
static std::optional<Proxy> ProxyForClearnetPrivateBroadcast(Network clearnet)
{
    ...
    if (onion_arg != "" && onion_arg != "0" && GetProxy(NET_ONION, tor_proxy)) {
        ...
        // return the generic proxy provided in -proxy which may be an ordinary SOCKS5 proxy
        // that has nothing to do with Tor.
        return tor_proxy;
    }
    return std::nullopt;
}
...
MaybePickPrivateBroadcastNetwork(bool prev_was_private_broadcast, size_t num_needed, size_t num_opened)
{
    ...
    if (g_reachable_nets.Contains(NET_ONION)) {
        ...
        for (const Network clearnet : {NET_IPV4, NET_IPV6}) {
            if (ProxyForClearnetPrivateBroadcast(clearnet).has_value()) {
                nets.push_back(clearnet);
            }
        }
    }
...
void CConnman::ThreadOpenConnections(const std::vector<std::string> connect)
{
    ...
    while (!interruptNet)
    {
        ...
        open_private_broadcast_to =
            MaybePickPrivateBroadcastNetwork(open_private_broadcast_to.has_value(),
...
void CConnman::PrivateBroadcastSub(size_t n)
{
    ...
    // if (m_private_broadcast_connections_to_open > n) {
    //     m_private_broadcast_connections_to_open -= n;
    // } else {
    //     m_private_broadcast_connections_to_open = 0;
    // }
    // m_private_broadcast_connections_to_open is maintained in a loose way.
    // It may end up more or less than needed. In any case make sure that it
    // does not become negative. Thus we don't do the simpler
    // m_private_broadcast_connections_to_open.fetch_sub(n).
