...
bool TxOrphanage::AddTx(const CTransactionRef& tx, NodeId peer)
{
    ...
    LogDebug(BCLog::TXPACKAGES, "stored orphan tx %s (wtxid=%s) (mapsz %u outsz %u)\n", hash.ToString(), wtxid.ToString(),
...
void TxOrphanage::EraseForPeer(NodeId peer)
{
    ...
    if (nErased > 0) LogDebug(BCLog::TXPACKAGES, "Erased %d orphan tx from peer=%d\n", nErased, peer);
}
...
void TxOrphanage::EraseForBlock(const CBlock& block)
{
    ...
    if (vOrphanErase.size()) {
        ...
        LogDebug(BCLog::TXPACKAGES, "Erased %d orphan tx included or conflicted by block\n", nErased);
    }
}
